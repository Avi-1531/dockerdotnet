...
namespace ClinicManagementDB_Business
{
    public class clsMedicalRecord
    {
        ...
        private clsMedicalRecord(int? MedicalRecordID, string Diagnosis, string Prescription, string Notes, short CreatedByUserID, DateTime CreatedAt)
        {
        ...
        {
            this.MedicalRecordID = (int?)clsMedicalRecordData.AddNewMedicalRecord(this.Diagnosis, this.Prescription, this.Notes, this.CreatedByUserID, this.CreatedAt);
            return (this.MedicalRecordID != -1);
        }
        private bool _UpdateMedicalRecord()
            => clsMedicalRecordData.UpdateMedicalRecord(this.MedicalRecordID, this.Diagnosis, this.Prescription, this.Notes, this.CreatedByUserID, this.CreatedAt);
        public static clsMedicalRecord Find(int? MedicalRecordID)
        {
            ...
            bool IsFound = clsMedicalRecordData.GetMedicalRecordByID(MedicalRecordID, ref Diagnosis, ref Prescription, ref Notes, ref CreatedByUserID, ref CreatedAt);

            if(IsFound)
                return new clsMedicalRecord(MedicalRecordID, Diagnosis, Prescription, Notes, CreatedByUserID, CreatedAt);
            else
                return null;
        }
        public bool Save()
